...
import companion.support.v8.lang.NoSuchPropertyExceptionCompat;

/**
 * Internal class to automatically generate a Property for a given class/name pair, given the
 * specification of {@link PropertyCompat#of(java.lang.Class, java.lang.Class, java.lang.String)}
 */
@SuppressWarnings({"rawtypes", "unchecked"})
class ReflectiveProperty<T, V> extends PropertyCompat<T, V> {
    ...
    public ReflectiveProperty(Class<T> propertyHolder, Class<V> valueType, String name) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (!typesMatch(valueType, fieldType)) {
                                throw new NoSuchPropertyExceptionCompat("Underlying type (" + fieldType + ") " +
                                        "does not match Property type (" + valueType + ")");
                            }
                            return;
                        } catch (NoSuchFieldException e5) {
                            // no way to access property - throw appropriate exception
                            throw new NoSuchPropertyExceptionCompat("No accessor method or field found for"
        ...
        if (!typesMatch(valueType, getterType)) {
            throw new NoSuchPropertyExceptionCompat("Underlying type (" + getterType + ") " +
                    "does not match Property type (" + valueType + ")");
        }
