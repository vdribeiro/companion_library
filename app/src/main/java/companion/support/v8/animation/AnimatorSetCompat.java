...
/**
 * This class plays a set of {@link AnimatorCompat} objects in the specified order. Animations
 * can be set up to play together, in sequence, or after a specified delay.
 *
 * <p>There are two different approaches to adding animations to a <code>AnimatorSet</code>:
 * either the {@link AnimatorSetCompat#playTogether(AnimatorCompat[]) playTogether()} or
 * {@link AnimatorSetCompat#playSequentially(AnimatorCompat[]) playSequentially()} methods can be called to add
 * a set of animations all at once, or the {@link AnimatorSetCompat#play(AnimatorCompat)} can be
 * used in conjunction with methods in the {@link AnimatorSetCompat.Builder Builder}
 ...
 */
public final class AnimatorSetCompat extends AnimatorCompat {
     ...
     */
    private ArrayList<AnimatorCompat> mPlayingSet = new ArrayList<AnimatorCompat>();
     ...
     */
    private HashMap<AnimatorCompat, Node> mNodeMap = new HashMap<AnimatorCompat, Node>();
    ...
    private ValueAnimatorCompat mDelayAnim = null;
     ...
     */
    public void playTogether(AnimatorCompat... items) {
        if (items != null) {
            mNeedsSort = true;
     ...
     * @param items The animations that will be started simultaneously.
     */
    public void playTogether(Collection<AnimatorCompat> items) {
        if (items != null && items.size() > 0) {
            mNeedsSort = true;
            Builder builder = null;
            for (AnimatorCompat anim : items) {
                if (builder == null) {
                    builder = play(anim);
                } else {
                    builder.with(anim);
     ...
     */
    public void playSequentially(AnimatorCompat... items) {
        if (items != null) {
            mNeedsSort = true;
            if (items.length == 1) {
                play(items[0]);
     ...
     * @param items The animations that will be started one after another.
     */
    public void playSequentially(List<AnimatorCompat> items) {
        if (items != null && items.size() > 0) {
            mNeedsSort = true;
            if (items.size() == 1) {
                play(items.get(0));
     ...
     */
    public ArrayList<AnimatorCompat> getChildAnimations() {
        ArrayList<AnimatorCompat> childList = new ArrayList<AnimatorCompat>();
     ...
     * of this AnimatorSet that take targets ({@link ObjectAnimatorCompat} and
     * AnimatorSet).
     *
     ...
     */
    public Builder play(AnimatorCompat anim) {
        if (anim != null) {
            mNeedsSort = true;
    ...
    public void start() {
        ...
        if (mStartDelay <= 0) {
            ...
            mDelayAnim = ValueAnimatorCompat.ofFloat(0f, 1f);
            mDelayAnim.setDuration(mStartDelay);
            mDelayAnim.addListener(new AnimatorListenerAdapterCompat() {
                boolean canceled = false;
                public void onAnimationCancel(AnimatorCompat anim) {
                    canceled = true;
                }
                public void onAnimationEnd(AnimatorCompat anim) {
                    if (!canceled) {
    ...
    @Override
    public AnimatorSetCompat clone() {
        final AnimatorSetCompat anim = (AnimatorSetCompat) super.clone();
        /*
        ...
        anim.mStarted = false;
        anim.mPlayingSet = new ArrayList<AnimatorCompat>();
        anim.mNodeMap = new HashMap<AnimatorCompat, Node>();
    ...
    private static class DependencyListener implements AnimatorListener {
        ...
        private AnimatorSetCompat mAnimatorSet;
        ...
        public DependencyListener(AnimatorSetCompat animatorSet, Node node, int rule) {
         ...
         */
        public void onAnimationCancel(AnimatorCompat animation) {
        }

        /**
         * An end event is received - see if this is an event we are listening for
         */
        public void onAnimationEnd(AnimatorCompat animation) {
         ...
         */
        public void onAnimationRepeat(AnimatorCompat animation) {
        }

        /**
         * A start event is received - see if this is an event we are listening for
         */
        public void onAnimationStart(AnimatorCompat animation) {
         ...
         */
        private void startIfReady(AnimatorCompat dependencyAnimation) {
    ...
    private class AnimatorSetListener implements AnimatorListener {
        ...
        AnimatorSetListener(AnimatorSetCompat animatorSet) {
    ...
    private static class Node implements Cloneable {
        public AnimatorCompat animation;
         ...
         */
        public Node(AnimatorCompat animation) {
     ...
     * AnimatorSetCompat#play(AnimatorCompat) play()} method of <code>AnimatorSet</code> is to make it possible
     * to express the dependency relationships of animations in a natural way. Developers can also
     * use the {@link AnimatorSetCompat#playTogether(AnimatorCompat[]) playTogether()} and {@link
     * AnimatorSetCompat#playSequentially(AnimatorCompat[]) playSequentially()} methods if these suit the need,
     * but it might be easier in some situations to express the AnimatorSet of animations in pairs.
     * <p/>
     * <p>The <code>Builder</code> object cannot be constructed directly, but is rather constructed
     * internally via a call to {@link AnimatorSetCompat#play(AnimatorCompat)}.</p>
     * <p/>
     ...
     * </pre>
     * <p/>
     * <p>Note in the example that both {@link Builder#before(AnimatorCompat)} and {@link
     * Builder#after(AnimatorCompat)} are used. These are just different ways of expressing the same
     * relationship and are provided to make it easier to say things in a way that is more natural,
     * depending on the situation.</p>
     * <p/>
     * <p>It is possible to make several calls into the same <code>Builder</code> object to express
     * multiple relationships. However, note that it is only the animation passed into the initial
     * {@link AnimatorSetCompat#play(AnimatorCompat)} method that is the dependency in any of the successive
    ...
    public class Builder {
         ...
         * {@link AnimatorSetCompat#play(AnimatorCompat)} call that created this <code>Builder</code> object.
         *
         * @param anim The animation that will play when the animation supplied to the
         * {@link AnimatorSetCompat#play(AnimatorCompat)} method starts.
         */
        public Builder with(AnimatorCompat anim) {
         ...
         *
         * @param anim The animation that will play when the animation supplied to the
         * {@link AnimatorSetCompat#play(AnimatorCompat)} method ends.
         */
        public Builder before(AnimatorCompat anim) {
            Node node = mNodeMap.get(anim);
            if (node == null) {
                node = new Node(anim);
                mNodeMap.put(anim, node);
         ...
         * {@link AnimatorSetCompat#play(AnimatorCompat)} method to play.
         */
        public Builder after(AnimatorCompat anim) {
            Node node = mNodeMap.get(anim);
            if (node == null) {
                node = new Node(anim);
                mNodeMap.put(anim, node);
        ...
        public Builder after(long delay) {
            ...
            ValueAnimatorCompat anim = ValueAnimatorCompat.ofFloat(0f, 1f);
